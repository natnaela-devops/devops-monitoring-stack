...
Data Prepper 2.16.0 does not use the unsupported Prometheus sink from the earlier experiment. RED and reporting metrics are generated by the validated Prometheus rules under `prometheus/rules/`.
...
4. Prometheus;
5. Alertmanager;
6. process-exporter on the observability host;
7. node_exporter on monitored Linux hosts where required;
8. RKE2 Collector exporters;
